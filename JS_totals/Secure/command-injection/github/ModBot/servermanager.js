...
module.exports = {
  ...
  async execute(message, args, extra) {
    ...
    const argv = ['systemctl', action, serviceName];

    this.logger.info(`Servermanager: User ${message.author.tag} attempting to execute: sudo ${argv.join(' ')}`);
    message.channel.send({ content: `Attempting to ${action} server "${serverDetails.display_name}" (service: ${serviceName})...` });

    execFile('sudo', argv, { windowsHide: true }, (error, stdout, stderr) => {
      if (error) {
        this.logger.error(`Servermanager: Error executing command "sudo ${argv.join(' ')}": ${error.message}`);
      ...
      if (stdout) {
        this.logger.info(`Servermanager: stdout: ${stdout}`);
        outputMessage += `Output:\n\`\`\`\n${stdout}\n\`\`\`\n`;
      }
      if (stderr) {
        this.logger.warn(`Servermanager: stderr (might be informational): ${stderr}`);
