...
/** Git hook runner HTTP — VULNERABLE (shimmed real sinks) */
...
const server = http.createServer((req, res) => {
  ...
  const repo = String(parsed.query.repo || '');  // ⚠️ untrusted
  const hook = String(parsed.query.hook || '');  // ⚠️ untrusted
  // ⚠️ Vulnerable: concatenated command string incorporating repo and hook
  const cmd = 'git --git-dir=' + repo + ' ' + hook;
  cp.exec(cmd, (err, stdout) => {
    res.setHeader('content-type','application/json');
    res.end(JSON.stringify({ ok: !err, simulated: true, cmd, sample: 'git_hook_http' }));
  });
});
server.listen(8101, () => console.log('Listening http://127.0.0.1:8101/git/hook'));
