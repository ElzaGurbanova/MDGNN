...
/** Rsync wrapper CLI — VULNERABLE (shimmed real sinks) */
const cp = require('child_process'); // shimmed
// Usage: node rsync_cli.js <src> <dest> [flags...]
const src = String(process.argv[2] || '');      // ⚠️ untrusted
const dest = String(process.argv[3] || '');     // ⚠️ untrusted
const flags = process.argv.slice(4).join(' ');  // ⚠️ untrusted
const cmd = 'rsync ' + flags + ' ' + src + ' ' + dest; // ⚠️ concatenation
cp.exec(cmd, (err, stdout) => {
  console.log('simulated:', !err, 'cmd=', cmd);
});
