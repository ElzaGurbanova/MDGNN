...
// --- Hardened: canonical upload root + safe resolver ---
var UPLOAD_ROOT = fs.realpathSync(path.resolve(options.uploadDir));
function safeResolve() {
  var candidate = path.resolve.apply(path, [UPLOAD_ROOT].concat([].slice.call(arguments)));
...
exports.show = function(req, res) {
  try {
    var filename = path.basename(req.params.id || '');
    // only allow known server-defined versions
    var version = req.query.v;
    if (version && !options.imageVersions[version]) {
      version = null;
    }
    ...
    var filepath = version
      ? safeResolve(version, filename)
      : safeResolve(filename);
    ...
    return fs.unlink(filepath, function (ex) {
        Object.keys(options.imageVersions).forEach(function (version) {
          try {
            ...
            fs.unlink(versionpath, function(){ /* ignore */ });
          } catch (e) { /* ignore */ }
        });
