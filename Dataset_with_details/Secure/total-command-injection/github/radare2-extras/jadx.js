...
const walk = async (dir, filelist = []) => {
  ...
  for (var file of files) {
    ...
    const stat = fs.statSync(filepath); // sync to match rest of file
...
async function decompile (target, mode, arg) {
  ...
  if (!fs.existsSync(outdir)) {
    ...
    const options = {}; // customize stdio if desired
...
function check () {
  ...
  const res = execFileSync(cmd[0], cmd.slice(1), { encoding: 'utf8' });
