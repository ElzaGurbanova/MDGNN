...
export function servePublicMiddleware(server, publicFiles) {
  ...
  return function viteServePublicMiddleware(req, res, next) {
    // Check against in-memory set of known public files; skip internal/import requests; allow ?url to be transformed
    if (
...
export function serveStaticMiddleware(server) {
  ...
  return function viteServeStaticMiddleware(req, res, next) {
    // Only serve non-HTML, non-directory requests; let HTML fall through; skip internal requests
...
export function serveRawFsMiddleware(server) {
  ...
  return function viteServeRawFsMiddleware(req, res, next) {
    // Serve /@fs/ paths from the real FS root (with allow-list checks via shouldServe)
 ...
 * Deprecated signature kept for compatibility: (url, server)
 */
...
export function checkLoadingAccess(config, p) {
  if (isFileLoadingAllowed(config, slash(p))) {
    return 'allowed'
  }
  if (isFileReadable(p)) {
    return 'denied'
...
export function checkServingAccess(url, server) {
...
export function respondWithAccessDenied(url, server, res) {
