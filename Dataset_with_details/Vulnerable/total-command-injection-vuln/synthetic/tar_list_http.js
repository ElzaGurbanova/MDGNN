...
/** Tar archive lister HTTP — VULNERABLE (shimmed real sinks) */
const http = require('http');
const url = require('url');

const cp = require('child_process'); // shimmed to dry-run
const server = http.createServer((req, res) => {
  const parsed = url.parse(req.url, true);
  if (parsed.pathname !== '/tar/list') { res.statusCode = 404; return res.end('not found'); }
  const file = String(parsed.query.file || ''); // ⚠️ untrusted
  const flags = String(parsed.query.flags || ''); // e.g., '--wildcards' (⚠️ untrusted)
  // ⚠️ Vulnerable: build a shell string from untrusted input
  const cmd = 'tar -tf ' + file + ' ' + flags;
  cp.exec(cmd, (err, stdout) => {
    res.setHeader('content-type', 'application/json');
    res.end(JSON.stringify({ ok: !err, simulated: true, cmd, sample: 'tar_list_http' }));
  });
});
server.listen(8100, () => console.log('Listening http://127.0.0.1:8100/tar/list'));
