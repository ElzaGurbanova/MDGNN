...
/** ImageMagick convert HTTP — VULNERABLE (shimmed real sinks) */
...
const server = http.createServer((req, res) => {
  ...
  const input = String(parsed.query.input || '');    // ⚠️ untrusted
  const ops = String(parsed.query.ops || '-resize 50%'); // ⚠️ untrusted
  const output = String(parsed.query.output || 'out.jpg'); // ⚠️ untrusted
  const cmd = 'convert ' + input + ' ' + ops + ' ' + output; // ⚠️ concatenation
  cp.exec(cmd, (err) => {
    res.setHeader('content-type','application/json');
    res.end(JSON.stringify({ ok: !err, simulated: true, cmd, sample: 'convert_http' }));
  });
});
server.listen(8103, () => console.log('Listening http://127.0.0.1:8103/img/convert'));
